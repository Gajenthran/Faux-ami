...
const Game = ({ socket, user, users, gameState, onFullscreen}) => {
  ...
  const renderTitle = (width, height, top, left, text, textStyle, img, imgX, imgY, transform, pinX, pinY ) => {
    return (
      <div 
        className="post-it"
  ...
  const onPlaceWord = (userId) => {
    ...
    if(usersWords.get(userId).includes(keyword))
      return

    setUsersWords(new Map(
      usersWords.set(userId, [...usersWords.get(userId), keyword])
    ))
    setKeyword(null)
  }
  ...
  const onRemoveWord = (userId, word) => {
    ...
    setUsersWords(new Map(
      usersWords.set(userId, newUserWord)
    ))
  }

  const onVote = (userId) => {
    if(userId !== socket.id && !validVote)
      socket.emit('game:vote', { userId })
  }
      ...
      >
        {users.map((usr, index) => 
          <div 
            key={index}
            className={`post-it ${
              !validVote && usr.id !== socket.id ? 
              'underline-player' : null}`
            }
            style={{
              position: "relative",
              left: "0%",
          ...
          >
            ...
            >
              <div 
                className="pin-icon" 
                style={{
                  backgroundColor: usr.id === socket.id ?
                    "rgb(111, 240, 71)" : "rgb(221, 39, 39)",
                  border: usr.id === socket.id ?
                    "2px solid rgb(63, 175, 30)": "2px solid rgb(170, 8, 8)",
                }}
              > </div>
              <img 
                alt="avatar-img"
        ...
        {users.map((usr, index) => 
          ...
          >
            ...
            <div className="users-score">
              ...
              <div className="users-current-score"> 
                +{usr.currentScore} 
              </div>
            </div>
          </div>
        ...
        {users.map((usr, index) => 
          <div 
            ...
            style={{
              ...
              display: "flex",
              flexDirection: "column",
              backgroundColor: usr.id === gameState.currentPlayer ? 
                "rgb(248, 232, 212)" : "rgb(216, 202, 184)",
            }}
          ...
          >
            ...
            >
              <div 
                ...
                style={{
                  ...
                  border: usr.id === socket.id ?
                    "2px solid rgb(63, 175, 30)": "2px solid rgb(170, 8, 8)",
                  animation: usr.id === gameState.currentPlayer ?
                    "popup .5s linear" : null,
                }}
              > </div>
              <img 
                alt="avatar-img"
            ...
            <div className="keywords-users">
              {usersWords.get(usr.id).map((userWord, index) => (
                <div
                  key={index}
                  onClick={() => onRemoveWord(usr.id, userWord)}
                > 
                  <p> {userWord} </p>
                </div>
              ))}
        ...
        {keywords && keywords.map((word, index) => {
          return (
            <div key={index}>
              <p 
                style={{
                  backgroundColor: word === keyword ?
                    "rgb(248, 232, 212)" : "rgb(216, 202, 184)",
                  boxShadow: word === keyword ?
                    "0 0 0 rgba(0, 0, 0, .5)" : "5px 5px 0 rgba(0, 0, 0, .5)"
                }}
  ...
  const renderAdvice = () => {
    ...
    return (
      <div className="advice-div">
        {suspicious && suspicious.map((suspect, index) => {
